...
export default class ClassSubjectService {
  ...
  async assignSubjectToClass(data) {
    return prisma.$transaction(async (tx) => {
      const [classe, subject, annee, teacher] = await Promise.all([
        ...
        data.teacherId ? tx.teacher.findUnique({ 
          where: { id: data.teacherId },
          include: { teacherSubjects: { where: { subjectId: data.subjectId } } }
      ...
      if (data.teacherId && !teacher) throw new Error("Professeur introuvable");
      if (data.teacherId && teacher.teacherSubjects.length === 0) {
      ...
      if (existing) throw new Error("Cette matière est déjà assignée à cette classe");
  ...
  async getClassSubjects(classId, anneeScolaireId) {
    ...
    if (!classe) throw new Error("Classe introuvable");
    const annee = await prisma.anneeScolaire.findUnique({ where: { id: anneeScolaireId } });
    if (!annee) throw new Error("Année scolaire introuvable");
