...
export default class ClassRoute {
  ...
  setupRoutes() {
    this.router.get("/stats", this.authMiddleware.protect(["admin"]), (ctx) =>
      this.controller.getStats(ctx)
    );
    this.router.get(
      "/teacher/:teacherId",
      this.authMiddleware.protect(["admin", "professeur"]),
      (ctx) => this.controller.getClassesByTeacher(ctx)
    );
    this.router.get("/", this.authMiddleware.protect(["admin"]), (ctx) =>
      this.controller.getAllClasses(ctx)
    );
    ...
    this.router.put("/:id", this.authMiddleware.protect(["admin"]), (ctx) =>
      this.controller.updateClass(ctx)
    );
    this.router.patch(
      "/:id/delete",
      this.authMiddleware.protect(["admin"]),
      (ctx) => this.controller.deleteClass(ctx)
    );
    this.router.patch(
      "/:id/restore",
      this.authMiddleware.protect(["admin"]),
      (ctx) => this.controller.restoreClass(ctx)
    );
  }
